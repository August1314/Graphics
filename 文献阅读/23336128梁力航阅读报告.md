...
论文的主要贡献可以概括为三个互补的面向。首先，它提出了一个可扩展的框架，使任何预训练LLM或多模态LLM都能融入矢量图形任务，这个框架通过可学习的语义Token机制让模型在理解和生成阶段都保持对几何、外观与语言的精确对齐。其次，作者设计了模块化架构，在指令和参数层面实现解耦，通过专门的矢量指令编码器、语义标签与渲染辅助接口，将SVG要素以更规整的方式输入LLM，保证生成输出的语义一致性。最后，他们构建了规模达250k高质量矢量作品与580k指令样本的SVGX-SFT数据集，结合自动化清洗、再标注以及多轮指令模板，填补了矢量领域缺乏高质量指令数据的空白，为后续模型的监督微调提供扎实基础。这三项贡献彼此支撑，既有模型机制的创新，也有数据管线的突破，更强调在真实评价体系上与传统方法进行定量和定性对比。
