...
import { BezierCurveTool } from './tools/bezier_curve.js';
import { BSplineCurveTool } from './tools/bspline_curve.js';
import { BezierSurfaceTool } from './tools/bezier_surface.js';
...
class DrawingApp {
    ...
    registerTools() {
        ...
        bezierTool.setDocument(this.document);
        bezierTool.setStyle(this.currentStyle);
        bezierTool.on('shapeCreated', (data) => {
            this.document.addShape(data.shape);
        });
        this.tools.set('bezierCurve', bezierTool);

        const bsplineTool = new BSplineCurveTool();
        bsplineTool.setDocument(this.document);
        bsplineTool.setStyle(this.currentStyle);
        bsplineTool.on('shapeCreated', (data) => {
            this.document.addShape(data.shape);
        });
        this.tools.set('bsplineCurve', bsplineTool);

        const surfaceTool = new BezierSurfaceTool();
        surfaceTool.setDocument(this.document);
        surfaceTool.setStyle(this.currentStyle);
        surfaceTool.on('shapeCreated', (data) => {
            this.document.addShape(data.shape);
        });
        this.tools.set('bezierSurface', surfaceTool);
    ...
    initToolbar() {
        ...
        const tools = [
            ...
            { id: 'bezierCurve', icon: '♾️', label: 'Bézier', title: '绘制 Bézier 曲线' },
            { id: 'bsplineCurve', icon: '🌀', label: 'B 样条', title: '绘制 B 样条曲线' },
            { id: 'bezierSurface', icon: '🧊', label: '曲面', title: '绘制 Bézier 曲面' }
        ];
