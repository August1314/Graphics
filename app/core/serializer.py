...
def dump(scene: QGraphicsScene) -> Dict[str, Any]:
        ...
        try:
            ...
            if tname == "BrushPathItem":
                ...
                try:
                    ...
                    print(f"DEBUG: 使用基础导出: {data}")
                if data:
                    data["pen"] = encode_pen(it.pen())
                    data["type"] = "brush_path"
                    shapes.append(data)
                    print(f"DEBUG: 成功添加 BrushPathItem 到 shapes，当前 shapes 长度: {len(shapes)}")
                else:
                    print(f"DEBUG: data 为空，跳过")
                continue
            if tname == "CircleItem":
            ...
            try:
                ...
                if isinstance(it, BrushPathItem):
                    ...
                    if data:
                        ...
                        data["type"] = "brush_path"
                        shapes.append(data)
                        print(f"DEBUG: 成功添加 BrushPathItem 到 shapes")
                    else:
                        print(f"DEBUG: to_dict() 返回 None 或空数据")
                    continue
            except Exception as e:
                print(f"DEBUG: BrushPathItem isinstance 检查失败: {e}")
                # 尝试通过类名匹配
                if tname == "BrushPathItem":
                    print(f"DEBUG: 通过类名匹配找到 BrushPathItem")
                    try:
                        data = it.to_dict()  # type: ignore[attr-defined]
                        print(f"DEBUG: to_dict() 返回: {data}")
                        if data:
                        ...
                        print(f"DEBUG: BrushPathItem to_dict() 失败: {e2}")
                pass
            try:
    ...
    # 调试信息：如果 shapes 为空，打印场景中的对象信息
    if not shapes:
        print(f"DEBUG: 场景中有 {len(items)} 个对象")
        for i, it in enumerate(items):
            try:
                print(f"  [{i}] {type(it).__name__} - {it.__class__.__module__}")
            except Exception as e:
                print(f"  [{i}] 无法获取类型信息: {e}")
    return {
        "version": "1.0",
