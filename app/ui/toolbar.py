...
class ToolBar(QToolBar):
    ...
    def __init__(self, parent=None) -> None:
        ...
        # 颜色：标签 + 按钮
        lbl_color = QLabel("颜色")
        lbl_color.setAlignment(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignLeft)
        self._btn_color = QPushButton("选择")
        self._btn_color.setMinimumWidth(64)
        self._btn_color.setMinimumHeight(28)
        self._btn_color.clicked.connect(self._pick_quick_color)
        # 线宽标签 + 数字框
        lbl_width = QLabel("线宽")
        lbl_width.setAlignment(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignLeft)
        self._spin_width = QSpinBox()
        self._spin_width.setRange(1, 50)
        self._spin_width.setValue(2)
        self._spin_width.setMinimumWidth(60)
        self._spin_width.setMinimumHeight(28)
        self._spin_width.valueChanged.connect(lambda v: self.quickStrokeWidthChanged.emit(float(v)))
        # 线型（实线/虚线）：标签 + 下拉
        lbl_dash = QLabel("线型")
        lbl_dash.setAlignment(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignLeft)
        self._combo_dash = QComboBox()
        self._combo_dash.addItems(["实线", "虚线"])
        self._combo_dash.setMinimumWidth(76)
        self._combo_dash.setMinimumHeight(28)
        # 根据内容宽度自动调整
        try:
            self._combo_dash.setSizeAdjustPolicy(QComboBox.SizeAdjustPolicy.AdjustToContents)
        except Exception:
            pass
        # 居中文本（下拉和选择框）+ 垂直居中显示
        try:
